 ...
 *
 * View state is driven by `docIdFromUrl`:
 *   - null  → home (or transient uploading/processing during file pick)
 *   - "..." → workspace for that document
 *
 * Navigation goes through the URL: opening a doc pushes to
 * `/dashboard/analyzer/{docId}`, closing pops back to `/dashboard/analyzer`.
 * Browser back/forward/refresh/share-link all work as a result.
 */
export default function AnalyzerView({ docIdFromUrl }: { docIdFromUrl: string | null }) {
  ...
  // Label map: chunk_id → "N - TypeName" — built once on doc load, shared with ParsePanel
  ...
  // ── Derived: which doc the URL is asking for ───────────────────────────────
  const selectedDoc: Doc | null = docIdFromUrl
    ? docs.find(d => d.id === docIdFromUrl) ?? null
    : null;
  ...
  // ── Load doc data when URL points at a complete doc ────────────────────────
  // Replaces the old "openDoc on click" pattern. URL is the trigger now.
  // Bounce to home if the URL refers to a doc the user can't see.
  useEffect(() => {
  ...
  }, [docIdFromUrl, loadingDocs, docs.length]);

  // ── Fetch signed URL + chunk overlays + grounding for one doc ──────────────
  async function loadDocData(doc: Doc) {
  ...
  async function handleDeleteDoc(docId: string) {
    try {
      ...
      if (data.success) {
        ...
        if (selectedDoc?.id === docId) navigateHome();
  ...
  // ── URL has a docId but doc data hasn't resolved yet (loading) ─────────────
  if (view === "workspace" && !selectedDoc) {
    return (
      <div className="flex flex-1 items-center justify-center min-h-0">
        <div className="flex flex-col items-center gap-4">
          ...
          />
          <p className="text-sm font-medium" style={{ color: "var(--al-text-secondary)" }}>Loading document…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-1 min-h-0">
      <DocumentRail documents={docs} selectedId={selectedDoc?.id} onSelect={navigateToDoc} onDelete={handleDeleteDoc} onRetry={handleRetryDoc} loading={loadingDocs} />
        ...
        {view === "processing" && processingId && (
          <div className="flex items-center justify-center pt-8">
            <ProcessingStatus
              ...
              onCancel={() => { setProcessingId(null); fetchDocs(); }}
            />
          </div>
        )}
